...
class EventTargetImpl {
  ...
  _dispatch(event, targetOverride) {
    ...
    if (event.type !== "load" && target._defaultView) {
...
function invokeEventListeners(listeners, target, event) {
  ...
  for (let i = 0; i < handlers.length; ++i) {
    ...
    if (listeners.indexOf(listener) === -1 || event.eventPhase === Event.CAPTURING_PHASE && !listener.capture || event.eventPhase === Event.BUBBLING_PHASE && listener.capture) {
      continue;
    }
...
//# sourceMappingURL=EventTarget-impl-compiled.js.map