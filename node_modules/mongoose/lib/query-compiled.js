...
Query.prototype._find = function (callback) {
  ...
  var cb = function (err, docs) {
    ...
    if (!options.populate) {
      return options.lean === true ? callback(null, docs) : completeMany(_this.model, docs, fields, _this, null, callback);
    }
    ...
    _this.model.populate(docs, pop, function (err, docs) {
      ...
      return options.lean === true ? callback(null, docs) : completeMany(_this.model, docs, fields, _this, pop, callback);
    });
  };
...
Query.prototype._findOne = function (callback) {
  ...
  Query.base.findOne.call(_this, {}, function (err, doc) {
    ...
    if (!options.populate) {
      return options.lean === true ? callback(null, doc) : completeOne(_this.model, doc, null, projection, _this, null, callback);
    }
    ...
    _this.model.populate(doc, pop, function (err, doc) {
      ...
      return options.lean === true ? callback(null, doc) : completeOne(_this.model, doc, null, projection, _this, pop, callback);
    });
  });
};
...
Query.prototype._findAndModify = function (type, callback) {
  ...
  var cb = function (err, doc, res) {
    ...
    if (!options.populate) {
      return options.lean === true ? callback(null, doc) : completeOne(_this.model, doc, res, fields, _this, null, callback);
    }
    ...
    _this.model.populate(doc, pop, function (err, doc) {
      ...
      return options.lean === true ? callback(null, doc) : completeOne(_this.model, doc, res, fields, _this, pop, callback);
    });
  };
...
Query.prototype.update = function (conditions, doc, options, callback) {
  ...
  try {
    ...
    castedDoc = this._castUpdate(utils.clone(doc, $options), options && options.overwrite);
  } catch (err) {
    this._castError = castedQuery;
    if (callback) {
      callback(err);
...
Query.prototype._castUpdate = function _castUpdate(obj, overwrite) {
  ...
  while (i--) {
    ...
    if (val && val.constructor.name === 'Object' && (!overwrite || hasDollarKey)) {
  ...
  while (i--) {
    ...
    if (val && val.constructor.name === 'Object') {
      ...
      if (schema && schema.caster && op in castOps) {
        ...
        var _strict = 'strict' in this._mongooseOptions ? this._mongooseOptions.strict : (useNestedStrict && v.schema || this.schema).options.strict;
      ...
      var checkPath = key === '$each' || key === '$or' || key === '$and' ? pref : prefix + key;
      schema = this._getSchema(checkPath);

      var pathDetails = this.model.schema._getPathType(checkPath);
      var isStrict = 'strict' in this._mongooseOptions ? this._mongooseOptions.strict : (useNestedStrict && pathDetails.schema || this.schema).options.strict;
...
Query.prototype._applyPaths = function applyPaths() {
  ...
  switch (exclude) {
    case true:
      ...
      break;
    case false:
      if (this.model.schema && this.model.schema.paths['_id'] && this.model.schema.paths['_id'].options && this.model.schema.paths['_id'].options.select === false) {
        selected.push('-_id');
      }
