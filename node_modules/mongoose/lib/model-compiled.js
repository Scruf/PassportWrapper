...
Model.prototype.remove = function remove(options, fn) {
  ...
  if (this.$__.removing) {
    if (fn) {
      this.$__.removing.then(function (res) {
        fn(null, res);
      }, function (err) {
        fn(err);
      });
...
Model.discriminator = function discriminator(name, schema) {
  ...
  function clean(a, b) {
    ...
    if (!utils.deepEqual(a, b)) {
      throw new Error('Discriminator options are not customizable ' + '(except toJSON, toObject, _id)');
    }
  }
  ...
  return new Promise.ES6(function (resolve, reject) {
    ...
    _this.collection.mapReduce(null, null, o, function (err, ret, stats) {
      ...
      if (ret.findOne && ret.mapReduce) {
        ...
        var model = Model.compile('_mapreduce_' + ret.collectionName, Model.mapReduce.schema, ret.collectionName, _this.db, _this.base);
...
Model.geoNear = function (near, options, callback) {
  ...
  return new Promise.ES6(function (resolve, reject) {
    ...
    if (Array.isArray(near)) {
      if (near.length !== 2) {
        var error = new Error('If using legacy coordinates, must be an array ' + 'of size 2 for geoNear');
        reject(error);
        callback && callback(error);
        return;
      }
      ...
      if (near.type !== 'Point' || !Array.isArray(near.coordinates)) {
        error = new Error('Must pass either a legacy coordinate array or ' + 'GeoJSON Point to geoNear');
        reject(error);
        callback && callback(error);
        return;
      }
...
function subPopulate(docs, options, cache, cb) {
  ...
  return function (err) {
    ...
    pop.forEach(function (subOptions) {
      ...
      var schema = cache[prefix.substr(0, prefix.length - 1)] || _this.schema._getSchema(prefix);
      var subSchema;
      if (typeof subOptions.model === 'string') {
        subOptions.model = _this.model(subOptions.model);
      } else if (schema && schema.caster && schema.caster.options && schema.caster.options.ref) {
        ...
        } else if (subSchema && subSchema.caster && subSchema.caster.options && subSchema.caster.options.ref) {
      ...
      _populate(subOptions.model || _this, docs, utils.populate(subOptions), cache, next);
    });
  };
}
