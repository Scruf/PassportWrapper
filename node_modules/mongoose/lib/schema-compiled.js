...
reserved.collection = reserved.db = reserved.errors = reserved.init = reserved.isModified = reserved.isNew = reserved.get = reserved.modelName = reserved.save = reserved.schema = reserved.set = reserved.toObject = reserved.validate =
// hooks.js
...
Schema.interpretAsType = function (path, obj, options) {
  ...
  if (Array.isArray(type) || Array === type || type === 'array') {
    ...
    } else if (cast && (!cast[options.typeKey] || options.typeKey === 'type' && cast.type.type) && utils.getFunctionName(cast.constructor) === 'Object' && Object.keys(cast).length) {
