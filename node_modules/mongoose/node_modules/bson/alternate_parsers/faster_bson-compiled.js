...
function calculateElement(name, value) {
    ...
    switch (value.constructor) {/// removed all checks 'isBuffer' if Node.js Buffer class is present!?
        ...
        case ObjectID:
            /// we want these sorted from most common case to least common/deprecated;
            return len + 12;
        case String:
        ...
        case Number:
            if (Math.floor(value) === value) {
                ...
                if (value <= 2147483647 && value >= -2147483647) // 32 bit
                    return len + 4;else return len + 8; /// covers Long-ish JS integers as Longs!
        ...
        case Buffer:
            ///  replaces the entire Binary class!
            return len + 4 + value.length + 1;

        case Regex:
            /// these are handled as strings by serializeFast() later, hence 'gim' opts = 3 + 1 chars
            return len + Buffer.byteLength(value.source, 'utf8') + 1 + (value.global ? 1 : 0) + (value.ignoreCase ? 1 : 0) + (value.multiline ? 1 : 0) + 1;
        case Date:
        case Long:
        case Timestamp:
        case Double:
...
function packElement(name, value, checkKeys, buffer, i) {
    ...
    switch (value.constructor) {
        ...
        case Buffer:
            /// solves ALL of our Binary needs without the BSON.Binary class!?
        ...
        case Date:
            ...
            var lowBits = millis % 4294967296 | 0,
                highBits = millis / 4294967296 | 0;
...
function encodeIntBE(data, bytes) {
    ...
    var result = '';
    if (bytes >= 4) {
        result += String.fromCharCode(Math.floor(data / 0x1000000));data %= 0x1000000;
    }
    if (bytes >= 3) {
        result += String.fromCharCode(Math.floor(data / 0x10000));data %= 0x10000;
    }
    if (bytes >= 2) {
        result += String.fromCharCode(Math.floor(data / 0x100));data %= 0x100;
    }
...
//# sourceMappingURL=faster_bson-compiled.js.map