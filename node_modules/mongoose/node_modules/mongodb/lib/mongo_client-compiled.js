...
var serverOptions = ['poolSize', 'ssl', 'sslValidate', 'checkServerIdentity', 'sslCA', 'sslCert', 'sslKey', 'sslPass', 'autoReconnect', 'noDelay', 'keepAlive', 'connectionTimeoutMS', 'socketTimeoutMS', 'reconnectTries', 'reconnectInterval'];

var replsetOptions = ['ha', 'haInterval', 'replicaSet', 'secondaryAcceptableLatencyMS', 'connectWithNoPrimary', 'poolSize', 'ssl', 'checkServerIdentity', 'sslValidate', 'sslCA', 'sslCert', 'sslKey', 'sslPass', 'noDelay', 'keepAlive', 'connectTimeoutMS', 'socketTimeoutMS'];

var mongosOptions = ['ha', 'haInterval', 'poolSize', 'ssl', 'checkServerIdentity', 'sslValidate', 'sslCA', 'sslCert', 'sslKey', 'sslPass', 'noDelay', 'keepAlive', 'connectTimeoutMS', 'socketTimeoutMS', 'acceptableLatencyMS'];

var dbOptions = ['authSource', 'w', 'wtimeout', 'j', 'native_parser', 'forceServerObjectId', 'serializeFunctions', 'ignoreUndefined', 'raw', 'promoteLongs', 'bufferMaxEntries', 'readPreference', 'pkFactory', 'promiseLibrary', 'readConcern'];
...
var connect = function (url, options, callback) {
  ...
  if (replSetServersOptions.ssl || replSetServersOptions.sslValidate || replSetServersOptions.checkServerIdentity || replSetServersOptions.sslCA || replSetServersOptions.sslCert || replSetServersOptions.sslKey || replSetServersOptions.sslPass) {
  ...
  if (mongosOptions.ssl || mongosOptions.sslValidate || mongosOptions.checkServerIdentity || mongosOptions.sslCA || mongosOptions.sslCert || mongosOptions.sslKey || mongosOptions.sslPass) {
    ...
    var connectFunction = function (__server) {
      ...
      new Db(object.dbName, __server, { w: 1, native_parser: false, promiseLibrary: options.promiseLibrary }).open(function (err, db) {
        ...
        if (totalNumberOfServers == 0) {
          ...
          if (totalNumberOfMongodServers == 0 && totalNumberOfMongosServers == 0 && object.servers.length == 1 && (!object.rs_options.replicaSet || !object.rs_options.rs_name)) {
          ...
          } else if (totalNumberOfMongodServers > 0 || totalNumberOfMongosServers > 0 || object.rs_options.replicaSet || object.rs_options.rs_name) {
            ...
            var finalServers = object.servers.filter(function (serverObj) {
              ...
              return serverObj.domain_socket ? new Server(serverObj.domain_socket, 27017, object.server_options) : new Server(serverObj.host, serverObj.port, object.server_options);
            });
            ...
            if (totalNumberOfMongodServers > 0) {
              try {
                ...
                // direct connection
                if (totalNumberOfMongodServers == 1 && !object.rs_options.replicaSet && !object.rs_options.rs_name) {
