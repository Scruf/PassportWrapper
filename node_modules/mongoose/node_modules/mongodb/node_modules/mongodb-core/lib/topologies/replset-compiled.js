...
var connectHandler = function (self, state) {
  return function (server) {
    ...
    var processNewServer = function () {
      ...
      // Deal with events
      var events = ['error', 'close', 'timeout', 'connect', 'message', 'serverOpening', 'serverDescriptionChanged', 'serverHeartbeatStarted', 'serverHeartbeatSucceeded', 'serverHearbeatFailed', 'serverClosed'];
      // Remove any non used handlers
      events.forEach(function (e) {
        server.removeAllListeners(e);
      });
      ...
      if (state.initialConnectionServers.length == 0 && state.replState.connectingServersCount() == 0 && !state.replState.isPrimaryConnected() && !state.secondaryOnlyConnectionAllowed && state.replState.state == CONNECTING) {
